...
func TestCreateAccount_Success(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/accounts", bytes.NewBufferString(`{"document_number":"12345678900","balance":500}`))
	...
	if acc.Balance != 500 {
		t.Errorf("expected balance 500, got %f", acc.Balance)
	}
}
...
func TestCreateAccount_NegativeBalance(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/accounts", bytes.NewBufferString(`{"document_number":"123","balance":-100}`))
	w := httptest.NewRecorder()
	mux.ServeHTTP(w, req)
